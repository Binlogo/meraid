...
impl<'a> Layout<'a> {
    ...
    fn layout_flowchart(&self) -> LayoutResult {
        ...
        let all_targets: std::collections::HashSet<String> =
            self.diagram.edges.iter().map(|e| e.to.clone()).collect();

        let mut starting_nodes: Vec<String> = self
            .diagram
            .nodes
            .iter()
        ...
        let mut queue: Vec<(String, usize)> =
            starting_nodes.iter().map(|n| (n.clone(), 0)).collect();
        ...
        let mut layer_nodes: std::collections::HashMap<usize, Vec<String>> =
            std::collections::HashMap::new();
