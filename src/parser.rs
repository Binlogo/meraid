...
fn parse_flowchart_edge(
    ...
    for window in edge_parts.windows(2) {
        ...
        nodes
            ...
            .or_insert_with(|| Node::new(from_id, from_id));
        nodes
            .entry(to_id.to_string())
            .or_insert_with(|| Node::new(to_id, to_id));
...
fn parse_class(source: &str) -> anyhow::Result<Diagram> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("class") && line.contains("{") {
            let class_name = line.split_whitespace().nth(1).unwrap_or("").trim();
            ...
            if parts.len() == 2 {
                ...
                if !from.is_empty() {
                    nodes
                        ...
                        .or_insert_with(|| Node::new(from, from));
                }
                if !to.is_empty() {
                    nodes
                        .entry(to.to_string())
                        .or_insert_with(|| Node::new(to, to));
                }
...
fn parse_er(source: &str) -> anyhow::Result<Diagram> {
    ...
    for line in source.lines() {
        ...
        if let Some(dash_pos) = line.find("--") {
            ...
            if !from.is_empty()
                && !to.is_empty()
                && from.chars().all(|c| c.is_alphanumeric() || c == '_')
            {
                relationships.push(Relationship {
                    from: from.to_string(),
                    to: to.to_string(),
